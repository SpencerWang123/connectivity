...
#'
#'
wavelet_test_long=function(adj_mat,tuning_in=0,tuning_out=1,length_seq,memory=60,sample_memory,m=100,alpha=1)
{
  N=ncol(adj_mat)
  mean_mat=NULL
  sd_mat=NULL
  ...
  for(j in 1:m)
  {
    temp_connectivity=connectivity_seq(adj_mat,tuning_in = tuning_in,tuning_out = tuning_out,memory = memory,sample_memory =N,length_seq =N,seeds = j,alpha = alpha)
