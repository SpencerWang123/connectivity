...
connectivity_seq=function(adj_mat,tuning_in=0.5,tuning_out=0.5,alpha=1,seeds=10,memory,sample_memory,jump=0,length_seq)#
{
  ...
  for (i in 1:(length_seq-1))
  {
    ...
    #随机化当前连接，计算随机换后的连接水平
    expected_connectivity=sum(memory_in_degree)/sum(in_degree)
    ...
    if(rbinom(1,1,jump)>0)
    {
      ...
      expect_connect=c(expect_connect,0)
    }
    else
    {
      ...
      expect_connect=c(expect_connect,expected_connectivity)
    }
  }
  temp_node_list=c(temp_node_list,temp_node)
  node_seq_all=unlist(temp_node_list)
  results=c(list(node_seq_all),list(connect_record_smooth),list(expect_connect))
  names(results)=c("node_seq","connect_record_smooth","expect_connect")
  return(results)
}
